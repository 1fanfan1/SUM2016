 ...
 * RETURNS: None.
 */
static VOID AS5_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;
...
} /* End of 'AS5_RndShaderLoadTextFile' function */
 ...
 */
static CHAR * AS5_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  CHAR *text;
  INT flen;
...
UINT AS5_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      AS5_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    txt = AS5_RndShaderLoadTextFile(buf);
    if (txt == NULL)
    {
      AS5_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'AS5_RndShaderLoad' function */
...
VOID AS5_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'AS5_RndShaderFree' function */
